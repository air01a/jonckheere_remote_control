...
public:
  ...
  err_t2 begin(TwoWire *theWire = &Wire); //!< @return ERROR_NONE
  err_t2 setClockBuilderData(void);       //!< @return ERROR_NONE
  err_t2 setupPLL(si5351PLL_t pll, uint8_t mult, uint32_t num,
                 uint32_t denom);                   //!< @return ERROR_NONE
  err_t2 setupPLLInt(si5351PLL_t pll, uint8_t mult); //!< @return ERROR_NONE
  err_t2 setupMultisynth(uint8_t output, si5351PLL_t pllSource, uint32_t div,
                        uint32_t num, uint32_t denom); //!< @return ERROR_NONE
  err_t2 setupMultisynthInt(uint8_t output, si5351PLL_t pllSource,
                           si5351MultisynthDiv_t div); //!< @return ERROR_NONE

  err_t2 enableSpreadSpectrum(bool enabled);
  err_t2 enableOutputs(bool enabled);
  /*!
   * @param output Enables or disables output
   * @param div Set of output divider values (2^n, n=1..7)
   */
  err_t2 setupRdiv(uint8_t output, si5351RDiv_t div); //!< @return ERROR_NONE

private:
  si5351Config_t m_si5351Config;

  Adafruit_I2CDevice *i2c_dev = NULL; ///< Pointer to I2C bus interface
  err_t2 write8(uint8_t reg, uint8_t value);
  err_t2 read8(uint8_t reg, uint8_t *value);
  err_t2 writeN(uint8_t *data, uint8_t n);
